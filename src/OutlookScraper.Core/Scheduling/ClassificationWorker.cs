    ...
    ILogger<ClassificationWorker>? logger = null)
{
    ...
    private async Task ProcessOneAsync(QueuedItem item, CancellationToken ct)
    {
        try
        {
            ...
            if (email is null)
            {
                ...
                _logger?.LogDebug("Message {EntryId} no longer available.", item.EntryId);
                return;
            }
            ...
            {
                await NotifyAsync(outcome.Suggestion, item.Priority);
            }
        ...
        {
            _breaker.RecordFailure();
            _logger?.LogError(ex, "Unhandled error processing {EntryId}.", item.EntryId);
        }
        finally
        {
    ...
    /// </summary>
    private async Task NotifyAsync(EventSuggestion suggestion, ProcessingPriority priority)
    {
